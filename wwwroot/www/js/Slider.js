(function (window, TweenMax, $) {
    ...
    Slider.prototype.init = function (target) {
        ...
        this.nodes.$previous = this.nodes.$current.prev().length === 0 ? this.nodes.$slides.last() :this.nodes.$current.prev();
        ...
        if (this.settings.nextArrow) {
            ...
            if(this.nodes.$nextArrow.next().hasClass('next-view')){
                this.nodes.$nextArrow.next().find('img').attr("src", this.nodes.$next.find("> img").attr("src"));
                this.nodes.$nextArrow.next().find('h3').text(this.nodes.$next.find("h1").text());
            }
        }
        ...
        if (this.settings.previousArrow) {
            ...
            if(this.nodes.$previousArrow.next().hasClass('prev-view')){
                this.nodes.$previousArrow.next().find('img').attr("src", this.nodes.$previous.find("> img").attr("src"));
                this.nodes.$previousArrow.next().find('h3').text(this.nodes.$previous.find("h1").text());
            }
        }
        ...
        //generate arrows if needed
        this.autoGenerateArrows();
    ...
    Slider.prototype.setSlides = function ($to) {
        ...
        // if(this.nodes.$nextArrow.find().hasClass('next-view')){
        this.nodes.$nextArrow.find('.image-block img').attr("src",  this.nodes.$current.next().length === 0 ?
                                                            this.nodes.$slides.first().find("> img").attr("src") :
                                                            this.nodes.$current.next().find("> img").attr("src"));
        this.nodes.$previousArrow.find('.image-block img').attr("src",  this.nodes.$current.prev().length === 0 ? 
                                                                    this.nodes.$slides.last().find("> img").attr("src") :
                                                                    this.nodes.$current.prev().find("> img").attr("src"));
        this.nodes.$previousArrow.find('.prev-view h3').text( this.nodes.$current.prev().length === 0 ?
                                                            this.nodes.$slides.last().find("h1").text() :
                                                            this.nodes.$previous.find("h1").text());
        this.nodes.$nextArrow.find('.next-view h3').text(this.nodes.$current.next().length === 0 ?
                                                        this.nodes.$slides.first().find("h1").text() :
                                                        this.nodes.$next.find("h1").text());
        // }
    ...
    Slider.prototype.autoGenerateArrows = function () {
        this._type.autoGenerateArrows();
    }
    ...
    InlineMultiSlider.prototype.autoGenerateArrows = function () {
    ...
    AccordionSlider.prototype.initEvent = function () {
        ...
        if(this.settings.showArrows == true) {
            $(document).on("click", ".sliderArrowRight span", function () {
                _self._parent.next();
                _self.data.touchedClock = jQuery.now();
            });
            $(document).on("click", ".sliderArrowLeft span", function () {
    ...
    AccordionSlider.prototype.autoGenerateArrows = function () {
        if (this.settings.showArrows === true) {
            var spanNext = document.createElement("span");
            var divNext = document.createElement("div");
            spanNext.className = "fa fa-angle-right";
            divNext.className = ("col-sm-1 sliderArrowRight " + this.settings.arrowsClass);
            divNext.appendChild(spanNext);
            this.nodes.$slider.after(divNext);

            var spanPrev = document.createElement("span");
            var divPrev = document.createElement("div");
            spanPrev.className = "fa fa-angle-left";
            divPrev.className = ("col-sm-1 sliderArrowLeft " + this.settings.arrowsClass);
            divPrev.appendChild(spanPrev);
            this.nodes.$slider.before(divPrev);

            this.nodes.$slider.addClass("col-sm-10");
        }
    }
    ...
    BookSlider.prototype.autoGenerateArrows = function () {
